...
// ----- Handler: POST /drivers/{driver_id}/complete -----

func (handler *DriverHTTPHandler) handleCompleteRide(w http.ResponseWriter, r *http.Request) {
	...
	in := ports.CompleteRideInput{
		DriverID: driverID,
		RideID:   req.RideID,
		FinalLocation: ports.GeoPoint{
	...
	// complete the ride and obtain the result
	res, err := handler.svc.CompleteRide(ctxWithTimeout, in)
	if err != nil {
		...
		handler.httpError(ctxWithTimeout, w, http.StatusInternalServerError, "failed to complete the ride", err)
		return
	}
