...
// ----- Handler: POST /drivers/{driver_id}/online -----

func (handler *DriverHTTPHandler) handleGoOnline(w http.ResponseWriter, r *http.Request) {
	...
	// change the status to online and obtain the result
	res, err := handler.svc.GoOnline(ctxWithTimeout, in)
	if err != nil {
		...
		handler.httpError(ctxWithTimeout, w, http.StatusInternalServerError, "failed to go online", err)
		return
	}
